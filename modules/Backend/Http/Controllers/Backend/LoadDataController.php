...
class LoadDataController extends BackendController
{
    ...
    protected function loadPosts(Request $request)
    {
        ...
        // $data['results'] = $query->get();
        $data['results'] = $paginate->map(function ($post) {
            $json_metas = $post->json_metas;
            $parent_id = $json_metas['parent'] ?? null;
            $parent_name = null;
            if ($parent_id) {
                $parent_name = Post::find($parent_id)->title;
            }
            return [
                'id' => $post->id,
                'text' => $post->text,
