...
class Post extends Model implements Feedable
{
    ...
    use PostTypeModel, HasFactory, QueryCacheable, UseUUIDColumn, SoftDeletes;
    ...
    {
        return $this->taxonomies()->where('taxonomy', '=', 'authors');
    }
    ...
    public function scopePublishedSiteMap(Builder $query)
    {
        return $query->where(function ($query) {
            $query->where(function ($query) {
                $query->where('date', '<=', Carbon::now())
    ...
    public function scopePublishedOrDraft(Builder $query)
    {
        $current_locale = app()->getLocale();
        return $query->where(function ($query) {
            $query->where(function ($query) {
            ...
            ->where('status', Post::STATUS_PUBLISH)
            ->orWhere('status', Post::STATUS_PREVIEW)
            ->orWhere('status', Post::STATUS_DRAFT);
    }
}
