...
class PluginUploader
{
    ...
    protected function uploadFileValidate(): bool
    {
        if ($this->file->getMimeType() != 'application/zip') {
            $this->throwError(trans_cms('cms::app.plugin_upload.error.mime_type'));
        }

        if ($this->file->extension() != 'zip') {
            $this->throwError(trans_cms('cms::app.plugin_upload.error.extension'));
        }
    ...
    protected function unzipFile(): bool
    {
        ...
        $this->throwError(trans_cms('cms::app.plugin_upload.error.unzip'));
    }
