...
@if(empty($options['type']))
    ...
    {{ Field::select(trans_cms('cms::app.post_type'), "{$name}[type]", $option) }}
@else
...
{{ Field::selectTaxonomy(trans_cms("cms::app.{$fieldName}"), "{$name}[{$fieldName}]", $option) }}
...
<div class="row">
    ...
    @endphp
    <div class="col-md-6">
        {{ Field::select(trans_cms('cms::app.sort_by'), "{$name}[sort_by]", $option) }}
    </div>
    <div class="col-md-6">
        @php
            $option = array_merge(
        ...
        {{ Field::select(trans_cms('cms::app.sort_order'), "{$name}[sort_order]", $option) }}
    </div>
</div>
...
{{ Field::text(trans_cms('cms::app.limit'), "{$name}[limit]", $option) }}
