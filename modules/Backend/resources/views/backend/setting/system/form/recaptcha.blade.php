<form method="post" action="{{ route('admin.setting.save') }}" class="form-ajax">
    ...
    <h5>{{ trans_cms('cms::app.google_recaptcha') }}</h5>

    @php
    $google_recaptcha = get_config('google_recaptcha');
    @endphp
    <div class="form-group">
        <label class="col-form-label" for="google_recaptcha">{{ trans_cms('cms::app.google_recaptcha') }}</label>
        <select name="google_recaptcha" id="google_recaptcha" class="form-control">
            <option value="1" @if($google_recaptcha == 1) selected @endif>{{ trans_cms('cms::app.enabled') }}</option>
            <option value="0" @if($google_recaptcha == 0) selected @endif>{{ trans_cms('cms::app.disabled') }}</option>
        </select>
    </div>

    <div class="form-group">
        <label class="col-form-label" for="google_recaptcha_key">{{ trans_cms('cms::app.google_recaptcha_key') }}</label>
    ...
    <div class="form-group">
        <label class="col-form-label" for="google_recaptcha_secret">{{ trans_cms('cms::app.google_recaptcha_secret') }}</label>
