        ...
        <div class="card-header border-bottom-0">
            <div class="d-flex">
                ...
                <div class="text-gray-6">
                    @if(!$network)
                    <button class="btn btn-primary active-theme" data-theme="{{ $theme->name }}"> {{ trans_cms('cms::app.activate') }}</button>
                    @endif

                    @if (config('juzaweb.theme.enable_upload') && $theme->update)
                    <button class="btn btn-success update-theme" data-theme="{{ $theme->name }}"> {{ trans_cms('cms::app.update') }}</button>
                    @endif

                    @if(config('juzaweb.theme.enable_upload'))
                        <a href="javascript:void(0)" class="delete-theme text-danger" data-theme="{{ $theme->name }}">
                            {{ trans_cms('cms::app.delete') }}
                        </a>
                    @endif
                </div>
            </div>
