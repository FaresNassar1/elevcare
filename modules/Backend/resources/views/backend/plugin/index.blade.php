        ...
        <div class="col-md-6">
            <div class="btn-group float-right">
                @if(config('juzaweb.plugin.enable_upload'))
                    <a href="{{ route('admin.plugin.install') }}" class="btn btn-success" data-turbolinks="false"><i class="fa fa-plus-circle"></i> {{ trans_cms('cms::app.add_new') }}</a>
                @endif
            </div>
        </div>
    </div>
        ...
        <div class="col-md-3">
            <form method="post" class="form-inline">
                ...
                <select name="bulk_actions" id="bulk-actions" class="form-control w-60 mb-2 mr-1">
                    <option value="">{{ trans_cms('cms::app.bulk_actions') }}</option>
                    <option value="activate">{{ trans_cms('cms::app.activate') }}</option>
                    <option value="deactivate">{{ trans_cms('cms::app.deactivate') }}</option>
                    @if(config('juzaweb.plugin.enable_upload'))
                    <option value="delete">{{ trans_cms('cms::app.delete') }}</option>
                    @endif
                </select>

                <button type="submit" class="btn btn-primary px-2 mb-2" id="apply-action">{{ trans_cms('cms::app.apply') }}</button>
            </form>
        </div>
        ...
        <div class="col-md-9">
            <form method="get" class="form-inline" id="form-search">
                <div class="form-group mb-2 mr-1">
                    <label for="search" class="sr-only">{{ trans_cms('cms::app.search') }}</label>
                    <input name="search" type="text" id="search" class="form-control" placeholder="{{ trans_cms('cms::app.search') }}" autocomplete="off">
                </div>

                <div class="form-group mb-2 mr-1">
                    <label for="status" class="sr-only">{{ trans_cms('cms::app.status') }}</label>
                    <select name="status" id="status" class="form-control select2-default">
                        <option value="">{{ trans_cms('cms::app.all_status') }}</option>
                        <option value="1">{{ trans_cms('cms::app.enabled') }}</option>
                        <option value="0">{{ trans_cms('cms::app.disabled') }}</option>
                    </select>
                </div>

                <button type="submit" class="btn btn-primary mb-2">{{ trans_cms('cms::app.search') }}</button>
            </form>
        </div>
    </div>
        ...
        <table class="table jw-table juzaweb-table">
            <thead>
                <tr>
                    ...
                    <th data-field="name" data-width="25%" data-formatter="nameFormatter">{{ trans_cms('cms::app.name') }}</th>
                    <th data-field="description">{{ trans_cms('cms::app.description') }}</th>
                    <th data-field="version" data-width="10%">{{ trans_cms('cms::app.version') }}</th>
                    <th data-width="15%" data-field="status" data-formatter="statusFormatter" data-align="center">{{ trans_cms('cms::app.status') }}</th>
                </tr>
            </thead>
        </table>
    </div>
