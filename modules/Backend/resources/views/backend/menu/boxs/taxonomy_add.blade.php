...
<ul class="nav nav-tabs" role="tablist">
    <li class="nav-item">
        <a class="nav-link active" href="#box-{{ $key }}-latest" data-toggle="tab">{{ trans_cms('cms::app.latest') }}</a>
    </li>

    <li class="nav-item">
        <a class="nav-link" href="#box-{{ $key }}-search" id="box-{{ $key }}-search-label" data-toggle="tab">{{ trans_cms('cms::app.search') }}</a>
    </li>
</ul>
...
<div class="tab-content">
    ...
    <div class="tab-pane fade p-2" id="box-{{ $key }}-search" role="tabpanel" aria-labelledby="box-{{ $key }}-tab">
        <input class="form-control menu-box-taxonomy-search" type="text" placeholder="{{ trans_cms('cms::app.search') }}" data-taxonomy="{{ $taxonomy->get('taxonomy') }}" data-key="{{ $key }}">
