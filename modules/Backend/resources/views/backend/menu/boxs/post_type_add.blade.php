<ul class="nav nav-tabs" role="tablist">
    <li class="nav-item">
        <a class="nav-link active" id="box-{{ $key }}-latest-label" href="#box-{{ $key }}-latest-tab" data-toggle="tab">{{ trans_cms('cms::app.latest') }}</a>
    </li>

    <li class="nav-item">
        <a class="nav-link" id="box-{{ $key }}-search-label" href="#box-{{ $key }}-search-tab" data-toggle="tab">{{ trans_cms('cms::app.search') }}</a>
    </li>
</ul>
...
<div class="tab-content">
    ...
    <div class="tab-pane fade p-2" id="box-{{ $key }}-search-tab" role="tabpanel" aria-labelledby="box-{{ $key }}-search-label">
        <input class="form-control menu-box-post-type-search" type="text" placeholder="{{ trans_cms('cms::app.search') }}" data-post_type="{{ $postType->get('key') }}" data-key="{{ $key }}">
