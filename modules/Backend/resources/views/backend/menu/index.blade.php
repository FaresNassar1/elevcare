...
@section('content')
    <div id="menu-container">
        <div class="row alert alert-light p-3 no-radius">
            ...
            <div class="col-md-6 form-select-menu">
                <div class="alert-default">
                    @if($menu)
                    {{ trans_cms('cms::app.select_menu_to_edit') }}:
                    <select name="id" class="w-25 form-control load-menu">
                        <option value="{{ $menu->id }}" selected>{{ $menu->name }}</option>
                    </select>

                    {{ trans_cms('cms::app.or') }}
                    @endif

                    <a href="javascript:void(0)" class="ml-1 btn-add-menu"><i class="fa fa-plus"></i> {{ trans_cms('cms::app.create_new_menu') }}</a>
                </div>
            </div>
            <div class="col-md-6 form-add-menu box-hidden">
                <form action="{{ route('admin.menu.store') }}" method="post" class="form-ajax form-inline">
                    <div class="form-group">
                        <input type="text" name="name" class="form-control" autocomplete="off" required placeholder="{{ trans_cms('cms::app.menu_name') }}">
                    </div>
                    <div class="form-group">
                    ...
                    <button type="submit" class="btn btn-primary"><i class="fa fa-plus"></i> {{ trans_cms('cms::app.add_menu') }}</button>
                </form>
            </div>
        </div>
        ...
        @if(!empty($menu))
            <div class="row mt-5">
                <div class="col-md-4">
                    <h5 class="mb-2 font-weight-bold">{{ trans_cms('cms::app.add_menu_items') }}</h5>

                    @do_action('juzaweb.add_menu_items')
                </div>

                <div class="col-md-8">
                    <h5 class="mb-2 font-weight-bold">{{ trans_cms('cms::app.menu_structure') }}</h5>
                    ...
                    <form action="{{ route('admin.menu.update', [$menu->id]) }}" method="post" class="form-ajax form-menu-structure">
                        ...
                        <div class="card">
                                ...
                                <div class="row">
                                    <div class="col-md-9">
                                        <div class="form-group row">
                                            <label for="name" class="col-sm-3">{{ trans_cms('cms::app.menu_name') }}</label>
                                            <div class="col-sm-9">
                                    ...
                                    <div class="col-md-3">
                                        <div class="btn-group float-right">
                                            <button type="submit" class="btn btn-primary"><i class="fa fa-save"></i> {{ trans_cms('cms::app.save') }}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card-footer">
                                <div class="btn-group">
                                    <a href="javascript:void(0)" class="text-danger delete-menu" data-id="{{ $menu->id }}">{{ trans_cms('cms::app.delete_menu') }}</a>
                                </div>
