                ...
                <div class="card-header">
                    ...
                    <div class="group-btns pull-right">
                        <a href="{{ route('admin.logs.error.download', [$log->date]) }}" class="btn btn-sm btn-success">
                            <i class="fa fa-download"></i> {{ trans_cms('Download') }}
                        </a>
                        <a href="javascript:void(0)" class="btn btn-sm btn-danger" id="delete-log">
                            <i class="fa fa-trash-o"></i> {{ trans_cms('Delete') }}
                        </a>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table id="entries" class="table mb-0">
                        <thead>
                            <tr>
                                <th>{{ trans_cms('ENV') }}</th>
                                <th style="width: 120px;">{{ trans_cms('Level') }}</th>
                                <th style="width: 65px;">{{ trans_cms('Time') }}</th>
                                <th>{{ trans_cms('Header') }}</th>
                                <th class="text-right">{{ trans_cms('Actions') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                                    ...
                                    @if ($entry->hasStack())
                                        <a class="btn btn-sm btn-light" role="button" data-toggle="collapse"
                                           href="#log-stack-{{ $key }}" aria-expanded="false" aria-controls="log-stack-{{ $key }}">
                                            <i class="fa fa-toggle-on"></i> {{ trans_cms('Stack') }}
                                        </a>
                                    @endif
                                    ...
                                    @if ($entry->hasContext())
                                        <a class="btn btn-sm btn-light" role="button" data-toggle="collapse"
                                           href="#log-context-{{ $key }}" aria-expanded="false" aria-controls="log-context-{{ $key }}">
                                            <i class="fa fa-toggle-on"></i> {{ trans_cms('Context') }}
                                        </a>
                                    @endif
                                </td>
                            </tr>
    ...
    <script>
        $(function () {
            ...
            btnDelete.on('click', function() {
                confirm_message(
                    "{{ trans_cms('Are you sure you want to delete this log file: :date ?', ['date' => $log->date]) }}",
                    function (result) {
                        if (!result) {
                            return false;
                        }
