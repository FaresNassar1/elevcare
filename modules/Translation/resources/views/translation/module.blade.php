...
@section('content')
        ...
        <div class="col-md-12">
            <div class="btn-group float-right">
                ...
                <a href="javascript:void(0)" class="btn btn-info" data-toggle="modal"
                    data-target="#key-add">{{ trans('cms::app.add_key') }}</a>
            </div>
        </div>
    </div>
            ...
            <form method="post" action="{{ route('admin.translations.type.add', [$type]) }}" class="form-ajax"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>{{ trans('cms::app.name') }}</label>
                            <input type="text" name="lang_custom_name" id="lang_custom_name" class="form-control" />
                        </div>
                    </div>
    ...
    <div class="modal fade" id="key-add" role="dialog" aria-labelledby="modal-add-title" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form method="post" action="{{ route('admin.translations.type.add.form.builder', [$type]) }}" class="form-ajax"
                data-success="add_language_success" data-notify="true">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modal-add-title">{{ trans('cms::app.add_language') }}</h5>
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label>{{ trans('cms::app.name') }}</label>
                            <input type="text" name="key" id="lang_custom_name" class="form-control" />
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        ...
        $('#locale').on('change', function() {
            var selectedLang = $(this).find('option:selected').text();
            $("#lang_custom_name").val(selectedLang);
        });
    </script>
@endsection
