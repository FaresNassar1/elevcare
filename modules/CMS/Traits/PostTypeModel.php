...
trait PostTypeModel
{
    ...
    public static function getStatuses($type = 'posts', $scheduled = false): array
    {
        $statuses = [
            'publish' => trans_cms('cms::app.publish'),
            'private' => trans_cms('cms::app.private'),
            'draft' => trans_cms('cms::app.draft'),
            'trash' => trans_cms('cms::app.trash'),
        ];
        if ($scheduled) {
            $statuses['scheduled'] = trans_cms('cms::app.scheduled');
        }
    ...
    public function attributeLabels(): array
    {
        return apply_filters(
            ...
            [
                'title' => trans_cms('cms::app.title'),
                'content' => trans_cms('cms::app.content'),
                'status' => trans_cms('cms::app.status'),
                'slug' => trans_cms('cms::app.slug'),
                'thumbnail' => trans_cms('cms::app.thumbnail'),
                'images' => trans_cms('cms::app.images'),
                'views' => trans_cms('cms::app.views'),
            ]
        );
    }
    ...
    public function syncTaxonomy(
        ...
        foreach ($taxonomies as $taxonomy) {
            $taxonomy->update(
                [
                    'total_post' => $taxonomy->posts()->where('status', '!=', 'preview')->count(),
                ]
            );
        }
    ...
    public function getRoute($absolute = true): bool | string
    {

        if ($this->getMeta('ctemplate') == "disable" || $this->type == 'authors') {
        ...
        if ($this->type == 'pages' || $this->type == 'posts') {
            return route('post', ["{$this->path}"], $absolute);
        }
        ...
        return route('post', ["{$permalink}/{$this->path}"], $absolute);
    }

    public function getLink($absolute = true): bool | string
    {
        if ($this->type == 'pages' || $this->type == 'posts') {
            return route('post', ["{$this->lang}{$this->path}"], $absolute);
        }

        $permalink = $this->getPermalink('base');
        if (empty($permalink)) {
            //return false;
            return route('post', ["{$this->lang}{$this->slug}"], $absolute);
        }

        return route('post', ["{$this->lang}/{$permalink}{$this->path}"], $absolute);
    }
