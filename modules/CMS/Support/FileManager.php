...
class FileManager
{
    ...
    public function save(): MediaFile
    {
        ...
        if ($this->resource_type != 'url' || $this->downloadFileUrlToServer) {
            ...
            if ($this->type == "youtube") {
                $extension = "youtube";
                $fileType = "url";
                $queryString = parse_url($this->resource, PHP_URL_QUERY);
                parse_str($queryString, $parameters);
                if (isset($parameters['v'])) {
                    $fileName = $parameters['v'];
                    $fileMime = $fileName;
                    //$embedUrl = 'https://www.youtube.com/embed/' . $parameters['v'];
                    $embedUrl = 'https://www.youtube.com/watch?v=' . $parameters['v'];
                    $this->resource = $embedUrl;
                }
            }
    ...
    protected function uploadUploadedFile(): MediaFile
    {
        ...
        try {
            ...
            $parts = explode('/', $uploadedFile->getMimeType());
            ...
            if ($media->isImage() && $uploadedFile->getMimeType() !== 'image/svg+xml') {
    ...
    protected function fileIsValid($file): bool
    {
        ...
        $config = config('juzaweb.filemanager.types.' . $fileType);
    ...
    {
        return trans_cms('cms::filemanager.error-' . $key);
    }
