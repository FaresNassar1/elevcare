...
function paginate_links($data, $view = null, $params = [])
{
    ...
    $queryParams = request()->only(['start_date', 'end_date', 'category', 'subpage', 'name', 'char']);
...
if (!function_exists('get_thumbnail_size')) {
    ...
    function get_thumbnail_size(string $postType, ?array $thumbnailSizes = null): array
    {
        ...
        $width = get_theme_config("thumbnail_sizes")[$postType]['width'] ?? $thumbnailSizes[$postType][array_key_first($thumbnailSizes[$postType] ?? [])]['width'] ?? '241';
        $height = get_theme_config("thumbnail_sizes")[$postType]['height'] ?? $thumbnailSizes[$postType][array_key_first($thumbnailSizes[$postType] ?? [])]['height'] ?? '241';
